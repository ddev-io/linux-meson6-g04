...
static int get_vt_node_info(struct aml_nftl_info_t *aml_nftl_info, addr_blk_t blk_addr)
{
	...
	valid_page = kcalloc(node_len_actual + 2, sizeof(*valid_page), GFP_KERNEL);
	if (!valid_page)
		return -ENOMEM;
...
int aml_nftl_initialize(struct aml_nftl_blk_t *aml_nftl_blk)
{
	...
	if (aml_nftl_info->cls.name) {
		aml_nftl_info->cls.class_attrs = nftl_class_attrs;
		error = class_register(&aml_nftl_info->cls);
		if (error) {
			printk("class register nand_class failed: %d\n", error);
			kfree(aml_nftl_info->cls.name);
			aml_nftl_info->cls.name = NULL;
		} else {
			aml_nftl_info->class_registered = true;
		}
	}
...
{
	if (aml_nftl_info->class_registered)
		class_unregister(&aml_nftl_info->cls);
	kfree(aml_nftl_info->cls.name);
	aml_nftl_free(aml_nftl_info->copy_page_buf);
	aml_nftl_free(aml_nftl_info->vtpmt);
	aml_nftl_free(aml_nftl_info->phypmt);
	aml_nftl_free(aml_nftl_info->aml_nftl_ops);
