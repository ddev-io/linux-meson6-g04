...
static int m3_nand_get_device(struct mtd_info *mtd, int new_state)
{
...
static void m3_nand_release_device(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
...
int m3_nand_boot_erase_cmd(struct mtd_info *mtd, int page)
{
	...
	struct nand_chip *chip = mtd->priv;
	loff_t ofs;
	int i, page_addr, status = NAND_STATUS_READY;
...
{
	struct aml_nand_chip *aml_chip = mtd_to_nand_chip(mtd);
	int status, i, write_page, configure_data, pages_per_blk, write_page_tmp, ran_mode = 0;
	int new_nand_type = 0;
	int en_slc = 0;
#ifdef MX_REVD
			...
			struct platform_device *pdev)
{
	if (pdev->dev.of_node) {
		...
		return match ? (struct aml_nand_device *)match->data : NULL;
	}
	return NULL;
}
#endif
...
static int m3_nand_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "(%p)\n", pdev);
	if (!aml_nand_dev) {
		dev_err(&pdev->dev, "no G04 NAND platform data\n");
		return -ENODEV;
	}

	/* Meson6 G04 uses the exact factory platform table below. */
	...
	for (i=1; i<aml_nand_dev->dev_num; i++) {
		...
		if (aml_chip) {
			...
			if(mtd){
				chip = mtd->priv;
				if(chip){
					m3_nand_get_device(mtd, FL_SHUTDOWN);
...
static struct mtd_partition normal_partition_info[] = {
	...
	{ .name = "recovery",  .offset = 104 * SZ_1M,  .size = 8 * SZ_1M },
...
/* The generic vendor DT parser is intentionally omitted in the G04 port. */
#ifdef CONFIG_OF
static const struct of_device_id amlogic_nand_dt_match[]={
...
{
	meson6_nand_cbus = ioremap(MESON6_CBUS_PHYS, MESON6_CBUS_SIZE);
	if (!meson6_nand_cbus)
		return -ENOMEM;
	nand_get_chip();
