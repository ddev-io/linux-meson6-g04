...
/* earlysuspend is not present in mainline */
/* pinmux configured locally for Meson6 */
#include <linux/pinctrl/consumer.h>

/* Meson6 CBUS is physically mapped by the vendor kernel.  The DT port maps
 * only the ranges used by NAND and keeps all register access in this driver. */
extern void __iomem *meson6_nand_cbus;
#define MESON6_CBUS_PHYS	0xc1100000
#define MESON6_CBUS_SIZE	0x10000
#define CBUS_REG_ADDR(reg)	(meson6_nand_cbus + ((reg) << 2))
#define READ_CBUS_REG(reg)	readl(CBUS_REG_ADDR(reg))
#define WRITE_CBUS_REG(reg, val)	writel((val), CBUS_REG_ADDR(reg))
#define WRITE_CBUS_REG_BITS(reg, val, start, len) do { \
	u32 __v = READ_CBUS_REG(reg); \
	u32 __m = ((BIT(len) - 1) << (start)); \
	WRITE_CBUS_REG(reg, (__v & ~__m) | (((val) << (start)) & __m)); \
} while (0)
#define SET_CBUS_REG_MASK(reg, mask) \
	WRITE_CBUS_REG((reg), READ_CBUS_REG(reg) | (mask))
#define CLEAR_CBUS_REG_MASK(reg, mask) \
	WRITE_CBUS_REG((reg), READ_CBUS_REG(reg) & ~(mask))
#define aml_read_reg32(addr)	readl((void __iomem *)(addr))
#define aml_set_reg32_mask(addr, mask) \
	writel(readl((void __iomem *)(addr)) | (mask), (void __iomem *)(addr))
#define aml_clr_reg32_mask(addr, mask) \
	writel(readl((void __iomem *)(addr)) & ~(mask), (void __iomem *)(addr))
...
#define PREG_HGPIO_EN_N	0x2018
#define PREG_HGPIO_O		0x2019
#define PREG_HGPIO_I		0x201a
#define PREG_PAD_GPIO3_EN_N	0x2015
#define PAD_PULL_UP_REG3	0x203d

struct pinmux_item_stub { u32 reg, clrmask, setmask; };
typedef struct pinmux_item_stub pinmux_item_t;
struct pinmux_set_stub { bool (*chip_select)(bool); pinmux_item_t *pinmux; };
typedef struct pinmux_set_stub pinmux_set_t;
#define PINMUX_REG(n)	(n)
#define PINMUX_END_ITEM { .reg = 0xffffffff }
static inline int pinmux_set(pinmux_set_t *set)
{
	pinmux_item_t *item;
	for (item = set->pinmux; item && item->reg != 0xffffffff; item++) {
		u32 val = READ_CBUS_REG(0x202c + item->reg);
		val &= ~item->clrmask;
		val |= item->setmask;
		WRITE_CBUS_REG(0x202c + item->reg, val);
	}
	return 0;
}
static inline int pinmux_clr(pinmux_set_t *set) { return 0; }
...
#define MESON_CPU_TYPE_MESON3    3
#define MESON_CPU_TYPE_MESON6    6
#define MESON_CPU_TYPE           MESON_CPU_TYPE_MESON6
#define NAND_ECC_FAIL(v)         (((v) >> 30) & 1)
...
struct aml_nand_chip {
	...
	struct { void *param; } nand_early_suspend;
    struct class      cls;
...
{
	/* Exact Meson6 vendor mux/pull-up sequence from mach-meson6/nand.h. */
	SET_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, 0x3ffff);
	SET_CBUS_REG_MASK(PAD_PULL_UP_REG3, 0xff | BIT(16));
	SET_CBUS_REG_MASK(0x202e, (0xf << 18) | BIT(17) |
			  (0x3 << 25) | BIT(27));
	CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_5, 0xf);
